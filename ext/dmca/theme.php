...
class DmcaTheme extends Themelet
{
    public function display_page(): void
    {
        ...
        Ctx::$page->add_block(new Block("DMCA takedown request form", $html, "main", 20));
    }

    public function display_submitted(): void
    {
        Ctx::$page->set_title("DMCA request submitted");
        $html = H2("Your request has been sent, staff will get back to you as soon as possible");
        Ctx::$page->add_block(new Block("DMCA request submitted", $html, "main", 20));
    }

    public function display_failed(): void
    {
        Ctx::$page->set_title("DMCA request failed to submit");
        $html = H2("DMCA request failed to send, please check if all fields are filled in");
        Ctx::$page->add_block(new Block("DMCA request failed to submit", $html, "main", 20));
    }
}
